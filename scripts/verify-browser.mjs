...
await expect(page.getByRole("button", { name: "Enable 3D animation" })).toHaveCount(0);
await expect(page.locator("#experience")).not.toHaveClass(/tour-static/);
await page.waitForTimeout(500);
const autoStart = await page.locator("canvas").screenshot();
await progress(0.2);
expect((await page.locator("canvas").screenshot()).equals(autoStart)).toBeFalsy();
await progress(0.08);
expect((await page.locator("canvas").screenshot()).equals(autoStart)).toBeTruthy();
await page.screenshot({ path: "test-results/reduced-motion-enabled.png" });
await page.getByRole("button", { name: "Pause 3D animation" }).click();
await expect(page.locator("canvas")).toHaveCount(0);
console.log("PASS: reduced-motion mobile automatic 3D, forward/reverse animation and pause.");
